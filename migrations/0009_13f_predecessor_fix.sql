-- v0.6.2: rebuild 13F rows after deterministic predecessor/CUSIP fix.
-- Keep the 13F signal gated out of HCS until validation is complete.
...
-- Remove any previously materialized 13F contribution from today's score rows.
-- Recompute the convergence bonus using only currently enabled positive families.
UPDATE scores
SET whale_score = 0,
